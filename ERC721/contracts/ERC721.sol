...
contract ERC721 {
    ...
    function _safeTransferFrom(address _from, address _to, uint256 _tokenId, bytes memory data) internal {
        ...
        balances[_to] += 1;
        (bool success, bytes memory returnData) = _to.call(abi.encodeWithSignature("onERC721Received(address,address,uint256,bytes)", msg.sender, _from, _tokenId, data));
        require(success && returnData.length == 32 && abi.decode(returnData, (bytes4)) == bytes4(keccak256("onERC721Received(address,address,uint256,bytes)" )), "onERC721Received failed");
    }
